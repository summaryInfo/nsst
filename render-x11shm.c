...
static struct image renderer_create_image(struct window *win, int16_t width, int16_t height) {
    ...
    if (rctx.has_shm) {
        ...
        if (!win->plat.shm_seg) {
            ...
            if (rctx.has_shm_pixmaps && win->plat.shm_pixmap)
...
void renderer_update(struct window *win, struct rect rect) {
    if (rctx.has_shm_pixmaps) {
        xcb_copy_area(con, win->plat.shm_pixmap, win->plat.wid, win->plat.gc, rect.x, rect.y,
                rect.x + win->cfg.left_border, rect.y + win->cfg.top_border, rect.width, rect.height);
    } else if (rctx.has_shm) {
        xcb_shm_put_image(con, win->plat.wid, win->plat.gc, STRIDE(win->plat.im.width), win->plat.im.height, rect.x, rect.y, rect.width, rect.height,
                rect.x + win->cfg.left_border, rect.y + win->cfg.top_border, TRUE_COLOR_ALPHA_DEPTH, XCB_IMAGE_FORMAT_Z_PIXMAP, 0, win->plat.shm_seg, 0);
    } else {
        xcb_put_image(con, XCB_IMAGE_FORMAT_Z_PIXMAP, win->plat.wid, win->plat.gc,
                STRIDE(win->plat.im.width), rect.height, win->cfg.left_border,
...
bool window_submit_screen(struct window *win, int16_t cur_x, ssize_t cur_y, bool cursor, bool marg) {
    ...
    if (win->plat.boundc) {
        optimize_bounds(win->plat.bounds, &win->plat.boundc, rctx.has_shm);
        for (size_t k = 0; k < win->plat.boundc; k++) {
            renderer_update(win, rect_scale_up(win->plat.bounds[k], win->char_width, win->char_depth + win->char_height));
        }
        win->plat.boundc = 0;
    }
...
void renderer_resize(struct window *win, int16_t new_cw, int16_t new_ch) {
    ...
    color_t c = win->bg_premul;
    if (delta_y > 0) image_draw_rect(win->plat.im,
            (struct rect) { 0, common_h, common_w, height - common_h }, c);
    if (delta_x > 0) image_draw_rect(win->plat.im,
